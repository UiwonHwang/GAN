...
# load parameters
with open("GAN_1D.yaml") as stream:
    try:
...
class GeneratorDistribution(object):
    ...
    def sample(self, N):
        return np.linspace(-self.range, self.range, N) + np.random.random(N) * 0.01  # 범위 내에서 uniform하게 뽑은 후 +-0.01의 noise를 줌
...
class GAN(object):
    ...
    def train(self):
        ...
        writer = tf.summary.FileWriter("C:/tmp/GAN_1D", session.graph)  # tensorboard를 위해 log를 기록
        ...
        for i, v in enumerate(self.D_params):
            with tf.variable_scope("assign_"+v.name[0:6]) as scope:
                assign_D = tf.assign(v,self.pre_D_weights[i])
        ...
        for step in range(1,self.num_steps+1):
            ...
            D_feed = {self.x: np.reshape(x, (self.batch_size, self.data_dim)),
                      self.z: np.reshape(z, (self.batch_size, self.data_dim)),
            ...
            # G를 update
            z = self.gen.sample(self.batch_size)
            G_feed = {self.z: np.reshape(z, (self.batch_size, 1))}
